// pages/common/member/examine/order/order.js
import {
  Member
...
Page({
  ...
  updateMobile: function (e) {
    ...
    memberModel.nianshenupdateCare(e.detail.value.mobile, that.data.orderData.id,(res) => {
      console.log(res)
      if (res.status == 1) {
        wx.showToast({
          title: '修改成功',
  ...
  toCancelOrder: function () {
    var that = this
    memberModel.nianshencancelOrder(that.data.orderData.id, (res) => {
      // console.log(res)
      if (res.status == 1) {
        that.toOrderDetail()
        wx.showToast({
